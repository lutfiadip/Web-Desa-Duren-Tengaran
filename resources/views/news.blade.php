...
@section('content')
    ...
    <section class="news-hero">
        ...
        <p>{{ $profile->news_page_description ?? 'Menyajikan informasi terbaru mengenai kegiatan kemasyarakatan, pengumuman resmi, dan perkembangan pembangunan di Desa Duren.' }}
        </p>
    </section>
    ...
    <div class="news-container">
        ...
        <div class="filter-bar">
            ...
            <form action="{{ route('news') }}" method="GET" class="search-form">
                ...
                <input type="text" name="search" class="search-input" placeholder="Cari judul atau isi berita..."
            ...
            <div class="category-select-wrapper">
                <select class="category-select" onchange="window.location.href = this.value">
                    ...
                    @foreach($categories as $category)
                        <option
                            value="{{ route('news', array_merge(request()->only('search'), ['category' => $category->slug])) }}"
        ...
        <div class="news-grid">
            @forelse($news as $item)
                <div class="news-card">
                    <div class="card-img-wrapper">
                        ...
                        <span
                            style="position: absolute; top: 15px; right: 15px; background-color: rgba(15, 23, 42, 0.7); backdrop-filter: blur(4px); color: var(--white); font-size: 0.75rem; font-weight: 700; padding: 4px 10px; border-radius: var(--radius-sm); display: flex; align-items: center; gap: 5px; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.15); z-index: 2;">
                    ...
                    <div class="card-body">
                        <div class="card-meta">
                            <span><i class="fa-solid fa-calendar-days"></i>
                                {{ \Carbon\Carbon::parse($item->published_at)->translatedFormat('d M Y') }}</span>
            ...
            @empty
                <div
                    style="grid-column: 1 / -1; text-align: center; padding: 60px 20px; background: var(--white); border-radius: var(--radius-lg); border: 1px solid var(--border-color); color: var(--text-muted);">
                    ...
                    <p style="font-size: 0.95rem; margin-bottom: 0;">Silakan cari dengan kata kunci lain atau pilih kategori
                        yang berbeda.</p>
                </div>
            @endforelse
        </div>
