...
@section('content')
    ...
    <nav style="margin-bottom: 25px; font-size: 0.9rem;">
        <ol style="list-style: none; padding: 0; display: flex; gap: 8px; align-items: center; color: var(--text-muted); margin: 0;">
            ...
            <li>
                <a href="{{ route('admin.news.index') }}" style="color: var(--primary-light); text-decoration: none; font-weight: 500; transition: var(--transition);">
                    Berita Desa
                </a>
            </li>
            <li><i class="fa-solid fa-chevron-right" style="font-size: 0.75rem; color: #94a3b8;"></i></li>
    ...
    <div style="display: grid; grid-template-columns: 7fr 5fr; gap: 30px; align-items: start;">
        ...
        <div class="card">
            <div class="card-header" style="flex-direction: column; align-items: start; gap: 6px;">
                ...
                <span style="font-size: 0.8rem; color: var(--text-muted);">Daftar kategori untuk pengelompokan berita desa di website publik.</span>
            </div>
                    ...
                    <div class="sortable-row" data-category-id="{{ $category->id }}" style="padding-left: 20px;">
                        <div class="row-drag-info">
                            <div>
                                ...
                                <span style="display: block; font-size: 0.8rem; color: var(--text-muted);">{{ $category->news()->count() }} Berita</span>
                            </div>
                        </div>
                        <div class="action-btns" style="justify-content: flex-end;">
                            <a href="{{ route('admin.news.categories.edit', $category->id) }}" class="btn-icon edit" title="Edit Nama Kategori">
                                <i class="fa-solid fa-pen-to-square"></i>
                            </a>
                            @if($category->news()->count() === 0)
                                <form action="{{ route('admin.news.categories.destroy', $category->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus kategori ini?')" style="display: inline;">
                                    @csrf
                                    @method('DELETE')
                            ...
                            @else
                                <span class="btn-icon delete" style="opacity: 0.4; cursor: not-allowed;" title="Kategori berisi berita (tidak dapat dihapus)">
                                    <i class="fa-solid fa-trash-can"></i>
                                </span>
                            @endif
                        </div>
        ...
        <div class="card">
            ...
            <form action="{{ route('admin.news.categories.store') }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" placeholder="Contoh: Pengumuman" value="{{ old('name') }}" required>
                    @error('name')
