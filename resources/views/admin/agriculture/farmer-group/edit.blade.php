...
@section('title', 'Sunting Kelompok Tani')
...
@section('content')
    ...
    <nav style="margin-bottom: 25px; font-size: 0.9rem;">
        <ol style="list-style: none; padding: 0; display: flex; gap: 8px; align-items: center; color: var(--text-muted); margin: 0;">
            ...
            <li><i class="fa-solid fa-chevron-right" style="font-size: 0.75rem; color: #94a3b8;"></i></li>
            <li style="color: var(--text-dark); font-weight: 600;">Sunting Kelompok Tani</li>
        </ol>
    </nav>
    ...
    <div class="card" style="max-width: 600px;">
        <div class="card-header" style="margin-bottom: 24px; padding-bottom: 16px; border-bottom: 1px solid var(--border-color); display: flex; align-items: center; justify-content: space-between;">
            <h2 style="font-size: 1.25rem; font-weight: 800; color: var(--text-dark); display: flex; align-items: center; gap: 10px; margin: 0;">
                <i class="fa-solid fa-edit" style="color: var(--primary-light);"></i> Sunting Kelompok Tani
            </h2>
        ...
        <form action="{{ route('admin.agriculture.farmer-group.update', $group->id) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $group->name) }}" placeholder="Masukkan nama kelompok tani..." required>
                @error('name')
            ...
            <div class="form-group">
                ...
                <input type="text" name="sector" id="sector" class="form-control @error('sector') is-invalid @enderror" value="{{ old('sector', $group->sector) }}" placeholder="Masukkan bidang usaha kelompok (contoh: Tanaman Pangan, Hortikultura, Peternakan)..." required>
                @error('sector')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" name="dusun" id="dusun" class="form-control @error('dusun') is-invalid @enderror" value="{{ old('dusun', $group->dusun) }}" placeholder="Masukkan wilayah kerja atau nama dusun..." required>
                @error('dusun')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div style="margin-top: 25px; border-top: 1px solid var(--border-color); padding-top: 15px;">
                <button type="submit" class="btn btn-primary">
                    <i class="fa-solid fa-save"></i> Perbarui Kelompok Tani
                </button>
            </div>
        </form>
    </div>
