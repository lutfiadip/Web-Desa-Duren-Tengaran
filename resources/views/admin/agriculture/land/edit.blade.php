...
@section('title', 'Sunting Statistik Lahan')
...
@section('content')
    ...
    <nav style="margin-bottom: 25px; font-size: 0.9rem;">
        <ol style="list-style: none; padding: 0; display: flex; gap: 8px; align-items: center; color: var(--text-muted); margin: 0;">
            ...
            <li><i class="fa-solid fa-chevron-right" style="font-size: 0.75rem; color: #94a3b8;"></i></li>
            <li style="color: var(--text-dark); font-weight: 600;">Sunting Statistik Lahan</li>
        </ol>
    </nav>
    ...
    <div class="card" style="max-width: 600px;">
        <div class="card-header" style="margin-bottom: 24px; padding-bottom: 16px; border-bottom: 1px solid var(--border-color); display: flex; align-items: center; justify-content: space-between;">
            <h2 style="font-size: 1.25rem; font-weight: 800; color: var(--text-dark); display: flex; align-items: center; gap: 10px; margin: 0;">
                <i class="fa-solid fa-edit" style="color: var(--primary-light);"></i> Sunting Statistik Lahan
            </h2>
        ...
        <form action="{{ route('admin.agriculture.land.update', $land->id) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="form-group">
                <label for="label">Label / Jenis Lahan <span style="color: red;">*</span></label>
                <input type="text" name="label" id="label" class="form-control @error('label') is-invalid @enderror" value="{{ old('label', $land->label) }}" placeholder="Masukkan jenis penggunaan lahan (contoh: Lahan Sawah, Perkebunan)..." required>
                @error('label')
            ...
            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 15px;">
                <div class="form-group">
                    ...
                    <input type="number" step="0.01" name="area" id="area" class="form-control @error('area') is-invalid @enderror" value="{{ old('area', $land->area) }}" placeholder="Masukkan angka luas lahan (contoh: 125.5)..." required>
                    @error('area')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <input type="text" name="unit" id="unit" class="form-control @error('unit') is-invalid @enderror" value="{{ old('unit', $land->unit) }}" placeholder="Contoh: Ha / m²..." required>
                    @error('unit')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="form-group">
                ...
                <input type="text" name="icon" id="icon" class="form-control @error('icon') is-invalid @enderror" value="{{ old('icon', $land->icon) }}" placeholder="Masukkan kelas ikon FontAwesome (contoh: fa-solid fa-wheat-awn)...">
            ...
            <div class="form-group">
                ...
                <input type="number" name="sort_order" id="sort_order" class="form-control @error('sort_order') is-invalid @enderror" value="{{ old('sort_order', $land->sort_order) }}" required>
                @error('sort_order')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
