    ...
    <form action="{{ route('admin.umkm.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
            <div class="form-group">
                ...
                <input type="text" id="title" name="title" class="form-control" placeholder="Masukkan nama usaha atau produk UMKM..." value="{{ old('title') }}" required>
            </div>

            <div class="form-group">
                <label for="owner_name">Nama Pemilik/Pengelola</label>
                <input type="text" id="owner_name" name="owner_name" class="form-control" placeholder="Masukkan nama pemilik atau pengelola usaha..." value="{{ old('owner_name') }}" required>
            </div>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
            ...
            <div class="form-group">
                ...
                <input type="text" id="whatsapp" name="whatsapp" class="form-control" placeholder="Masukkan nomor WhatsApp aktif pemesanan (contoh: 08123456789)..." value="{{ old('whatsapp') }}">
            </div>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
            <div class="form-group">
                ...
                <input type="text" id="instagram" name="instagram" class="form-control" placeholder="Masukkan username atau tautan profil Instagram..." value="{{ old('instagram') }}">
            </div>

            <div class="form-group">
                <label for="facebook">Nama Halaman Facebook (Opsional)</label>
                <input type="text" id="facebook" name="facebook" class="form-control" placeholder="Masukkan nama akun atau halaman Facebook..." value="{{ old('facebook') }}">
            </div>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
            <div class="form-group">
                ...
                <input type="text" id="operating_hours" name="operating_hours" class="form-control" placeholder="Masukkan jadwal atau jam operasional (contoh: Setiap Hari, 08:00 - 17:00)..." value="{{ old('operating_hours') }}">
            </div>

            <div class="form-group">
                <label for="google_maps_url">Link Google Maps Lokasi Usaha (Opsional)</label>
                <input type="url" id="google_maps_url" name="google_maps_url" class="form-control" placeholder="Masukkan tautan Google Maps lokasi usaha..." value="{{ old('google_maps_url') }}">
            </div>
        </div>

        <div class="form-group">
            <label for="description">Deskripsi Usaha & Produk</label>
            <textarea id="description" name="description" class="form-control" placeholder="Tuliskan deskripsi lengkap mengenai produk atau jasa yang ditawarkan, keunggulan, serta perkiraan harga..." required style="min-height: 120px;">{{ old('description') }}</textarea>
        </div>

        <div class="form-group">
            <label for="address">Alamat Lengkap Usaha</label>
            <input type="text" id="address" name="address" class="form-control" placeholder="Masukkan alamat lengkap lokasi usaha atau toko..." value="{{ old('address') }}" required>
        </div>
