...
@section('title', 'Sunting Organisasi')
...
@section('content')
    ...
    <nav style="margin-bottom: 25px; font-size: 0.9rem;">
        <ol style="list-style: none; padding: 0; display: flex; gap: 8px; align-items: center; color: var(--text-muted); margin: 0;">
            ...
            <li><i class="fa-solid fa-chevron-right" style="font-size: 0.75rem; color: #94a3b8;"></i></li>
            <li style="color: var(--text-dark); font-weight: 600;">Sunting Organisasi</li>
        </ol>
    </nav>
    ...
    <div class="card" style="max-width: 800px; margin: 0 auto;">
        <div class="card-header" style="margin-bottom: 24px; padding-bottom: 16px; border-bottom: 1px solid var(--border-color); display: flex; align-items: center; justify-content: space-between;">
            <h2 style="font-size: 1.25rem; font-weight: 800; color: var(--text-dark); display: flex; align-items: center; gap: 10px; margin: 0;">
                <i class="fa-solid fa-edit" style="color: var(--primary-light);"></i> Sunting Organisasi: {{ $institution->name }}
            </h2>
        ...
        <form action="{{ route('admin.organizations.update', $institution->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <div class="form-group">
                <label for="name">Nama Organisasi <span style="color: red;">*</span></label>
                <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $institution->name) }}" placeholder="Masukkan nama organisasi kemasyarakatan (contoh: Karang Taruna, Forum Pemuda)..." required>
                @error('name')
            ...
            <div class="form-group">
                ...
                <textarea name="description" id="description" rows="5" class="form-control @error('description') is-invalid @enderror" placeholder="Tuliskan profil singkat, kegiatan, dan fokus kemasyarakatan organisasi ini..." required>{{ old('description', $institution->description) }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="vision" id="vision" rows="3" class="form-control @error('vision') is-invalid @enderror" placeholder="Tuliskan visi organisasi jika ada (opsional)...">{{ old('vision', $institution->vision) }}</textarea>
                @error('vision')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="mission" id="mission" rows="4" class="form-control @error('mission') is-invalid @enderror" placeholder="Tuliskan poin-poin misi organisasi jika ada (opsional)...">{{ old('mission', $institution->mission) }}</textarea>
                @error('mission')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="address" id="address" rows="2" class="form-control @error('address') is-invalid @enderror" placeholder="Masukkan alamat lengkap kantor atau sekretariat organisasi...">{{ old('address', $institution->address) }}</textarea>
                @error('address')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                <div class="form-group">
                    <label for="contact_person">Nama Narahubung / Ketua Organisasi</label>
                    <input type="text" name="contact_person" id="contact_person" class="form-control @error('contact_person') is-invalid @enderror" value="{{ old('contact_person', $institution->contact_person) }}" placeholder="Masukkan nama ketua atau penanggung jawab organisasi...">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="contact" id="contact" class="form-control @error('contact') is-invalid @enderror" value="{{ old('contact', $institution->contact) }}" placeholder="Masukkan nomor telepon atau WhatsApp aktif (contoh: 081234567890)...">
                    @error('contact')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="form-group">
                ...
                <input type="email" name="email" id="email" class="form-control @error('email') is-invalid @enderror" value="{{ old('email', $institution->email) }}" placeholder="Masukkan alamat email resmi organisasi (opsional)...">
                @error('email')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div style="margin-top: 30px; border-top: 1px solid var(--border-color); padding-top: 15px; display: flex; gap: 10px;">
                <button type="submit" class="btn btn-primary">
                    <i class="fa-solid fa-save"></i> Perbarui Organisasi
                </button>
            </div>
        </form>
    </div>
