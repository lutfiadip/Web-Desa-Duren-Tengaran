    ...
    <div class="card" style="max-width: 800px; margin: 0 auto;">
        ...
        <form action="{{ route('admin.organizations.update', $institution->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea name="vision" id="vision" rows="3" class="form-control @error('vision') is-invalid @enderror" placeholder="Tulis visi organisasi jika ada...">{{ old('vision', $institution->vision) }}</textarea>
                @error('vision')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="mission" id="mission" rows="4" class="form-control @error('mission') is-invalid @enderror" placeholder="Tulis poin-poin misi organisasi jika ada...">{{ old('mission', $institution->mission) }}</textarea>
                @error('mission')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="address" id="address" rows="2" class="form-control @error('address') is-invalid @enderror" placeholder="Tulis alamat sekretariat organisasi jika ada...">{{ old('address', $institution->address) }}</textarea>
                @error('address')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                <div class="form-group">
                    ...
                    <input type="text" name="contact" id="contact" class="form-control @error('contact') is-invalid @enderror" value="{{ old('contact', $institution->contact) }}" placeholder="Masukkan 0812-3456-7890...">
                    @error('contact')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="email" name="email" id="email" class="form-control @error('email') is-invalid @enderror" value="{{ old('email', $institution->email) }}" placeholder="Masukkan ormas@duren.desa.id...">
                    @error('email')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
