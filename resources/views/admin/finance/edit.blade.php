            ...
            <div class="form-row" style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px;">
                <div class="form-group">
                    ...
                    <input type="number" name="year" id="year" class="form-control" value="{{ $report->year }}"
                        placeholder="Masukkan 4 digit tahun anggaran (contoh: 2026)..." required min="2000" max="2100">
                </div>
                        ...
                        @foreach($report->details->where('type', 'revenue') as $index => $detail)
                            <div class="dynamic-revenue-row" style="display: grid; grid-template-columns: 3fr 2fr auto; gap: 15px; align-items: flex-end; margin-bottom: 12px;">
                                <div class="form-group" style="margin: 0;">
                                    ...
                                    <input type="text" name="revenue_details[{{ $index }}][label]" class="form-control rev-label-input" value="{{ $detail->label }}" placeholder="Masukkan rincian atau sumber pendapatan (contoh: Pendapatan Asli Desa)..." required>
                                </div>
                                <div class="form-group" style="margin: 0;">
                                    <label style="font-size: 0.8rem; font-weight: 600;">Nominal Realisasi (Rp)</label>
                                    <input type="number" name="revenue_details[{{ $index }}][value]" class="form-control rev-val-input" value="{{ (int)$detail->value }}" placeholder="Masukkan nominal angka (contoh: 150000000)..." min="0" required>
                                </div>
                        ...
                        @foreach($report->details->where('type', 'spending') as $index => $detail)
                            <div class="dynamic-spending-row" style="display: grid; grid-template-columns: 3fr 2fr auto; gap: 15px; align-items: flex-end; margin-bottom: 12px;">
                                <div class="form-group" style="margin: 0;">
                                    ...
                                    <input type="text" name="spending_details[{{ $index }}][label]" class="form-control spend-label-input" value="{{ $detail->label }}" placeholder="Masukkan bidang belanja desa (contoh: Penyelenggaraan Pemerintahan)..." required>
                                </div>
                                <div class="form-group" style="margin: 0;">
                                    <label style="font-size: 0.8rem; font-weight: 600;">Nominal Realisasi (Rp)</label>
                                    <input type="number" name="spending_details[{{ $index }}][value]" class="form-control spend-val-input" value="{{ (int)$detail->value }}" placeholder="Masukkan nominal angka (contoh: 200000000)..." min="0" required>
                                </div>
                            ...
                            <div class="existing-doc-row" data-id="{{ $doc->id }}"
                                ...
                                <div class="form-group" style="margin: 0;">
                                    ...
                                    <input type="text" name="existing_document_titles[{{ $doc->id }}]" value="{{ $doc->title }}"
                                        class="form-control" placeholder="Masukkan judul dokumen (contoh: Laporan Realisasi APBDes)..." required>
                                </div>
                                <div class="form-group" style="margin: 0;">
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function addRevenueRow(label = '', value = '') {
                ...
                row.innerHTML = `
                    <div class="form-group" style="margin: 0;">
                        <label style="font-size: 0.8rem; font-weight: 600;">Nama Rincian / Indikator</label>
                        <input type="text" name="revenue_details[${revenueCounter}][label]" class="form-control rev-label-input" value="${label}" placeholder="Masukkan rincian atau sumber pendapatan (contoh: Pendapatan Asli Desa)..." required>
                    </div>
                    <div class="form-group" style="margin: 0;">
                        <label style="font-size: 0.8rem; font-weight: 600;">Nominal Realisasi (Rp)</label>
                        <input type="number" name="revenue_details[${revenueCounter}][value]" class="form-control rev-val-input" value="${value}" placeholder="Masukkan nominal angka (contoh: 150000000)..." min="0" required>
                    </div>
                    <button type="button" class="btn btn-danger btn-remove-revenue" style="padding: 10px 14px; margin-bottom: 0;">
                        <i class="fa-solid fa-trash-can"></i>
                    </button>
            ...
            function addSpendingRow(label = '', value = '') {
                ...
                row.innerHTML = `
                    <div class="form-group" style="margin: 0;">
                        <label style="font-size: 0.8rem; font-weight: 600;">Nama Bidang / Indikator</label>
                        <input type="text" name="spending_details[${spendingCounter}][label]" class="form-control spend-label-input" value="${label}" placeholder="Masukkan bidang belanja desa (contoh: Penyelenggaraan Pemerintahan)..." required>
                    </div>
                    <div class="form-group" style="margin: 0;">
                        <label style="font-size: 0.8rem; font-weight: 600;">Nominal Realisasi (Rp)</label>
                        <input type="number" name="spending_details[${spendingCounter}][value]" class="form-control spend-val-input" value="${value}" placeholder="Masukkan nominal angka (contoh: 200000000)..." min="0" required>
                    </div>
                    <button type="button" class="btn btn-danger btn-remove-spending" style="padding: 10px 14px; margin-bottom: 0;">
                        <i class="fa-solid fa-trash-can"></i>
                    </button>
            ...
            function addNewDocumentRow() {
                ...
                row.innerHTML = `
                    <div class="form-group" style="margin: 0;">
                        ...
                        <input type="text" name="document_titles[${indexCounter}]" class="form-control" placeholder="Masukkan judul atau nama laporan (contoh: Laporan Realisasi APBDes)..." required>
                    </div>
                    <div class="form-group" style="margin: 0;">
