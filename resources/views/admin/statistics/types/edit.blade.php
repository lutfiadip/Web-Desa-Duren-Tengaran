    ...
    <div class="card" style="padding: 30px; background-color: var(--white); border: 1px solid var(--border-color); border-radius: var(--radius-lg); box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05); max-width: 600px;">
        <form action="{{ route('admin.statistics.types.update', $type->id) }}" method="POST">
            ...
            <div class="form-group" style="margin-bottom: 20px;">
                ...
                <input type="text" name="name" id="name" class="form-control" value="{{ old('name', $type->name) }}" placeholder="Masukkan nama kategori statistik (contoh: Kelompok Umur, Pekerjaan)..." required style="width: 100%; padding: 10px 14px; border: 1px solid var(--border-color); border-radius: var(--radius-md); outline: none;">
            </div>
            ...
            <div class="form-group" style="margin-bottom: 20px;">
                ...
                <input type="text" name="slug" id="slug" class="form-control" value="{{ old('slug', $type->slug) }}" placeholder="Masukkan slug URL (contoh: kelompok-umur, jenis-pekerjaan)..." style="width: 100%; padding: 10px 14px; border: 1px solid var(--border-color); border-radius: var(--radius-md); outline: none;">
            ...
            <div class="form-group" style="margin-bottom: 20px;">
                ...
                <textarea name="description" id="description" rows="4" class="form-control" placeholder="Tuliskan deskripsi ringkas mengenai cakupan data statistik ini..." style="width: 100%; padding: 10px 14px; border: 1px solid var(--border-color); border-radius: var(--radius-md); outline: none;">{{ old('description', $type->description) }}</textarea>
            </div>
            ...
            <div class="form-group" style="margin-bottom: 30px; display: flex; align-items: center; gap: 10px;">
                <label class="switch">
                    <input type="checkbox" name="show_gender_percentage" value="1" {{ old('show_gender_percentage', $type->show_gender_percentage) ? 'checked' : '' }}>
                    <span class="slider"></span>
                </label>
