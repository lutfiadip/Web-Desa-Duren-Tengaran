...
@section('title', 'Kelola Lembaga Kemasyarakatan Desa')
...
@section('content')
    ...
    <nav style="margin-bottom: 25px; font-size: 0.9rem;">
        <ol style="list-style: none; padding: 0; display: flex; gap: 8px; align-items: center; color: var(--text-muted); margin: 0;">
            ...
            <li style="color: var(--text-dark); font-weight: 600;">Lembaga Kemasyarakatan Desa (LKD)</li>
        </ol>
    </nav>
        ...
        <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 15px;">
            <div>
                <h2 style="font-size: 1.2rem; font-weight: 800; color: var(--text-dark); display: flex; align-items: center; gap: 10px; margin: 0;">
                    <i class="fa-solid fa-landmark" style="color: var(--primary-light);"></i> Kelola Lembaga Kemasyarakatan Desa (LKD)
                </h2>
                <p style="font-size: 0.8rem; color: var(--text-muted); margin: 4px 0 0 0;">Kelola profil, visi misi, dan kepengurusan lembaga kemasyarakatan desa.</p>
            </div>
    ...
    <div class="card">
        <div class="card-header" style="margin-bottom: 20px; padding-bottom: 12px; border-bottom: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px;">
            <h3 style="font-size: 1.1rem; font-weight: 700; color: var(--text-dark); margin: 0;">Daftar Lembaga Desa</h3>
            <div style="display: flex; gap: 10px; align-items: center;">
                <form action="{{ route('admin.institutions.index') }}" method="GET" style="display: flex; gap: 5px;">
                    <input type="text" name="search" class="form-control" placeholder="Cari lembaga..." value="{{ request('search') }}" style="padding: 6px 12px; font-size: 0.85rem; height: auto; width: 180px;">
                    <button type="submit" class="btn btn-secondary btn-sm"><i class="fa-solid fa-search"></i></button>
                </form>
                <a href="{{ route('admin.institutions.create') }}" class="btn btn-primary btn-sm" style="display: flex; align-items: center; gap: 5px;">
                    ...
                    @forelse($institutions as $inst)
                        <tr>
                            <td>
                                ...
                                @else
                                    <div style="width: 45px; height: 45px; background: #e2e8f0; border-radius: var(--radius-sm); display: flex; align-items: center; justify-content: center; color: var(--text-muted); font-size: 0.75rem; font-weight: 700;">LKD</div>
                                @endif
                            </td>
                            ...
                            <td>
                                <div style="display: flex; gap: 6px; justify-content: center;">
                                    <a href="{{ route('admin.institutions.edit', $inst->id) }}" class="btn btn-secondary btn-sm" title="Edit Profil">
                                    ...
                                    <form action="{{ route('admin.institutions.destroy', $inst->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus lembaga ini? Semua data anggota di dalamnya juga akan terhapus.')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" style="text-align: center; color: var(--text-muted); padding: 30px;">
                                Belum ada data lembaga. Silakan tambah data baru.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
